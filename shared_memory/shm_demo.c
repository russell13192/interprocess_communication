...
int create_and_write_shared_memory(char *mmap_key, char *value, unsigned int size)
{
    ...
    /*
     * Creates a shared memory object in kernel space. If shared memory already
    ...
    /**
     * shm_reg is the address in the process's virtual address space, just like any other address.
     * The Linux paging mechanism maps this address to the starting address of the shared memory region
     * in kernel space. Any operation performed by the process on shm_reg address is actually the operation
     * performed in stared memory which resides in the kernel
     */
